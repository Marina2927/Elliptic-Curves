...
class ECurve:
    ...
    def __str__(self):
        return "y^2 + " + str(self.a1) + "xy + " + str(self.a3) + "y = x^3 + " + str(self.a2) + "x^2 + " + str(
            self.a4) + "x + " + str(self.a6)
...
class Elgamal:
    def __init__(self, curve, generator, secret):
        ...
        assert isinstance(self.curve, ECurve), "Curve must be elliptic"
    ...
    def decrypt(self, point, enc_mes, secret):
        q = secret * point
        mes = (enc_mes * mulinv(q.x, self.curve.field_power)) % self.curve.field_power
        return mes


el = ECurve(31991, 0, 0, 0, 31988, 1000)
G = EPoint(el, 0, 5585)
n = 32089
